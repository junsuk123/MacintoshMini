...
- **볼륨**: 화면 평면에서 기기를 시계 방향으로 기울이면 커지고 반시계 방향으로 기울이면 작아집니다. 기울인 동안 0.5초마다 한 단계씩 조절되며 부저로 피드백을 줍니다.
- **일시정지 / 재개**: 기기를 위아래로 흔들면 재생이 멈춥니다. 이때 오디오와 비디오가 같은 지점에서 동시에 정지하고, 한 번 더 흔들면 그 지점부터 싱크를 유지한 채 다시 재생됩니다. 흔드는 동안에는 볼륨이 흔들림에 영향을 받지 않도록 고정됩니다.

흔들기 감도(임계값, 크로싱 횟수, 시간 창, 쿨다운)는 [firmware/include/config.h](firmware/include/config.h)의 `Shake*` 상수로 조정합니다.
