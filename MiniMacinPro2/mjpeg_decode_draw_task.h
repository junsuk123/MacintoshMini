...
static int queueDrawMCU(JPEGDRAW *pDraw)
{
  ...
  //Serial.println("queueDrawMCU start.");

  ++_draw_queue_cnt;
  xQueueSend(_xqh, &j, portMAX_DELAY);
  //log_i("queueDrawMCU end.");
  //Serial.println("queueDrawMCU end.\n");
...
static void decode_task(void *arg)
{
  ...
  //Serial.println("decode_task end.");
...
static void draw_task(void *arg)
{
  ...
  //Serial.println("draw_task start");
...
void mjpeg_cleanup()
{
  ...
  // 만약 추가적으로 해제할 자원이 있다면 여기에 추가합니다.

  // xQueue 및 Task를 삭제하는 것은 FreeRTOS API를 사용해야 합니다.
  // 예를 들어, vTaskDelete 함수를 사용하여 Task를 삭제할 수 있습니다.
  // 그리고 vQueueDelete 함수를 사용하여 Queue를 삭제할 수 있습니다.

  // 예를 들어, 다음과 같이 Task를 삭제할 수 있습니다.
  if (_decodeTask != NULL)
  {
  ...
  // Queue를 삭제할 때에도 vQueueDelete 함수를 사용합니다.
