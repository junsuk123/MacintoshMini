...
int buttonPin_mjpeg = 17; // 버튼 핀 (마우스 핀)
int buttonPin_aac = 16; // 버튼 핀 (마우스 핀)
int buttonSyncPin=18;//오디오,mjpeg버튼 동기화 핀
int clicked=0;
...
{ 
  ...
  pinMode(buttonPin_mjpeg, INPUT);
  pinMode(buttonPin_aac, INPUT);
  pinMode(buttonSyncPin, OUTPUT);
  delay(1000);
...
void searchDirectory(File dir, int numTabs) {//SD카드에서 비디오 파일을 찾은 후 파일명을 구조체 배열에 저장
  while (true) {
    ...
    for (uint8_t i = 0; i < numTabs; i++) {
     ...
     if (!entry.isDirectory() && temp.endsWith(".mjpeg")&&strcmp("A.mjpeg",temp.c_str())&&strcmp("B.mjpeg",temp.c_str())) {
  ...
  if (!aFile || aFile.isDirectory()) {
    ...
    if (!vFile || vFile.isDirectory()) {
      ...
      if(aac_file_available){
        if (aac_file_available) {
          ret_val = aac_player_task_start(&aFile, AUDIOASSIGNCORE,buttonPin_aac);//오디오 재생 시작
        } else {
      ...
      // 영상 재생 중인지를 나타내는 변수
      bool isPlaying_mjpeg = true;
      digitalWrite(buttonSyncPin,LOW);//초기 설정
      while (vFile.available()) {

        // 현재 버튼 상태를 읽어옴
        bool isClicked_mjpeg = (digitalRead(buttonPin_mjpeg) == HIGH);
        if(isClicked_mjpeg&&isPlaying_mjpeg){
          while(isClicked_mjpeg = (digitalRead(buttonPin_mjpeg) == HIGH)){//눌려있을 동안 눌림 상태 유지
            Serial.println("button_mjpeg isClicked");
          }
          isPlaying_mjpeg = false;//재생 상태 업데이트-->일시정지
          digitalWrite(buttonSyncPin,HIGH);//전력 효율을 고려하여 일시 정지상태에서 HIGH신호로 전환
          delay(300);
        ...
        if(isClicked_mjpeg&&!isPlaying_mjpeg){
           while(isClicked_mjpeg = (digitalRead(buttonPin_mjpeg) == HIGH)){//눌려있을 동안 눌림 상태 유지
            Serial.println("button_mjpeg isClicked_again");
          }
            ...
            isPlaying_mjpeg = true;//재생 상태 업데이트-->영상 재생
            digitalWrite(buttonSyncPin,LOW);
